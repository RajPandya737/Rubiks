...
def rotation_matrix(axis: str, n: int):
    ...
    if axis == "x":
        return np.array(
            [[1, 0, 0], [0, angle_cos, -1 * angle_sin], [0, angle_sin, angle_cos]]
        )
    elif axis == "y":
        return np.array(
            [[angle_cos, 0, angle_sin], [0, 1, 0], [-1 * angle_sin, 0, angle_cos]]
        )
    elif axis == "z":
        return np.array(
            [[angle_cos, -1 * angle_sin, 0], [angle_sin, angle_cos, 0], [0, 0, 1]]
        )
...
class Cubie:
    def __init__(self, x: int, y: int, z: int, colors: tuple, type: str):
...
class Cube:
    ...
    def get_cubies_by_position(self, x, y, z):
        for cubie in self.cubies:
            if (
                cubie.position[0] == x
                and cubie.position[1] == y
                and cubie.position[2] == z
            ):
                return cubie
